...
export function registerBudgetOrderTools(server, client) {
    server.registerTool("list_budget_orders", {
        ...
        try {
            const resp = await client.get("/budgetorders", {
    ...
    server.registerTool("get_budget_order", {
        ...
        try {
            const resp = await client.get(`/budgetorders/${budgetOrderId}`);
            return {
                content: [
    ...
    server.registerTool("create_budget_order", {
        ...
        try {
            ...
            if (endDate)
                body.endDate = toISO8601(endDate);
            if (supplySources)
                body.supplySources = supplySources;
            if (orderNumber)
                body.orderNumber = orderNumber;
            if (clientName)
                body.clientName = clientName;
            if (primaryBuyerName)
                body.primaryBuyerName = primaryBuyerName;
            if (primaryBuyerEmail)
                body.primaryBuyerEmail = primaryBuyerEmail;
            if (billingEmail)
                body.billingEmail = billingEmail;
            const orgId = client.getOrgId();
            const resp = await client.post("/budgetorders", {
    ...
    server.registerTool("update_budget_order", {
        ...
        try {
            ...
            if (name)
                body.name = name;
            if (budgetAmount)
                body.budget = { amount: budgetAmount, currency: currency ?? "USD" };
            if (endDate)
                body.endDate = endDate;
            if (Object.keys(body).length === 0) {
                return { content: [{ type: "text", text: "No changes specified." }] };
            }
            const resp = await client.put(`/budgetorders/${budgetOrderId}`, { bo: body });
            return {
                content: [
...
//# sourceMappingURL=budget-orders.js.map