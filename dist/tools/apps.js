...
export function registerAppTools(server, client) {
    server.registerTool("search_apps", {
        ...
        try {
            ...
            const resp = await client.get("/search/apps", params);
    ...
    server.registerTool("get_app_eligibility", {
        ...
        try {
            ...
            const resp = await client.post(`/apps/${adamId}/eligibilities/find`, body);
    ...
    server.registerTool("get_app_details", {
        ...
        try {
            const resp = await client.get(`/apps/${adamId}`);
            const content = [
                { type: "text", text: `App details for Adam ID ${adamId}:` },
                { type: "text", text: JSON.stringify(resp.data, null, 2) },
            ];
            if (includeLocales) {
                const locales = await client.get(`/apps/${adamId}/locale-details`);
    ...
    server.registerTool("find_app_assets", {
        ...
        try {
            ...
            const resp = await client.post(`/apps/${adamId}/assets/find`, selector);
            return {
                content: [
    ...
    server.registerTool("search_geolocations", {
        ...
        try {
            ...
            if (geoIds?.length) {
                ...
                resp = await client.post("/search/geo", geoRequests);
            }
            else {
                ...
                resp = await client.get("/search/geo", params);
            }
            const text = resp.data
                .map((geo) => {
