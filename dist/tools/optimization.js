import { z } from "zod";
import { resolveDateRange } from "../utils/date-helpers.js";
...
export function registerOptimizationTools(server, client, config) {
    server.registerTool("get_campaign_snapshot", {
        ...
        try {
            const preset = (dateRange ?? "LAST_30_DAYS");
            const { startDate, endDate } = resolveDateRange(preset);
            const n = topN ?? 20;
            // Fetch all data in parallel
            const [campaignResp, adGroupsResp, keywordReport, searchTermReport, dailyReport] = await Promise.all([
                client.get(`/campaigns/${campaignId}`),
                client.get(`/campaigns/${campaignId}/adgroups`, { limit: "50" }),
                client.post(`/reports/campaigns/${campaignId}/keywords`, {
                    startTime: startDate,
                    endTime: endDate,
                    selector: buildSelector({ limit: n, sortBy: "localSpend", sortOrder: "DESCENDING" }),
                    returnRowTotals: true,
                }),
                client.post(`/reports/campaigns/${campaignId}/searchterms`, {
                    startTime: startDate,
                    endTime: endDate,
                    selector: buildSelector({ limit: n, sortBy: "localSpend", sortOrder: "DESCENDING" }),
                    returnRowTotals: true,
                }),
                client.post("/reports/campaigns", {
                    startTime: startDate,
                    endTime: endDate,
                    granularity: "DAILY",
            ...
            for (const row of keywords) {
                ...
                sections.push(`• "${m.keyword}" (${m.matchType}) — Bid: ${formatMoney(m.bidAmount)} — Spend: ${formatMoney(row.total.localSpend)} — Installs: ${row.total.totalInstalls} — CPA: ${formatMoney(row.total.avgCPA)}`);
            }
            // Top search terms
            sections.push(`\n=== TOP ${searchTerms.length} SEARCH TERMS BY SPEND ===`);
            for (const row of searchTerms) {
                const m = row.metadata;
                sections.push(`• "${m.searchTermText ?? "(unknown)"}" (matched "${m.keyword}" ${m.matchType}) — Spend: ${formatMoney(row.total.localSpend)} — Installs: ${row.total.totalInstalls} — CPA: ${formatMoney(row.total.avgCPA)}`);
            }
            // Daily trends
            ...
            if (granularity && Array.isArray(granularity)) {
                ...
                for (const day of granularity) {
                    sections.push(`${day.date}: Spend=${formatMoney(day.localSpend)} Impressions=${day.impressions ?? 0} Taps=${day.taps ?? 0} Installs=${day.totalInstalls ?? 0}`);
                }
            }
            return {
                content: [
                    ...
                    {
                        type: "text",
                        text: JSON.stringify({ campaign, adGroups, topKeywords: keywords, topSearchTerms: searchTerms, dailyTrends: dailyRows[0]?.granularity }, null, 2),
                    },
                ],
            };
        }
    ...
    server.registerTool("get_budget_analysis", {
        ...
        try {
            ...
            const [campaignsResp, reportResp] = await Promise.all([
                client.get("/campaigns", { limit: "200" }),
                client.post("/reports/campaigns", {
                    startTime: startDate,
                    endTime: endDate,
    ...
    server.registerTool("get_recommendations", {
        ...
        try {
            ...
            let adamId;
            try {
                const campaignsResp = await client.get("/campaigns", { limit: "1" });
            ...
            const response = await fetch("https://app-ads.apple.com/cm/api/v1/recommendation/keyword/find", {
                method: "POST",
                headers: {
            ...
            // Format output
            const sections = [`Apple Ads Keyword Recommendations (${items.length} total, sorted by expected installs):\n`];
            const top = items.slice(0, 30);
            for (const rec of top) {
                const kw = rec["keyword"] ?? "?";
                const mt = rec["matchType"] ?? "?";
                const bid = rec["suggestedBidAmount"]?.["amount"] ?? "?";
                const installs = rec["expectedInstalls"] ?? 0;
                const campaign = rec["campaignName"] ?? "?";
                const recId = rec["id"] ?? "";
    ...
    server.registerTool("dismiss_recommendation", {
