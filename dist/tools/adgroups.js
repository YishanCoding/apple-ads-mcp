    ...
    server.registerTool("list_ad_groups", {
        ...
        try {
            ...
            if (!campaignId) {
                ...
                resp = await client.post("/adgroups/find", selector);
            }
            else if (conditions?.length || sortBy) {
                const selector = buildSelector({ limit, offset, sortBy, sortOrder, conditions });
                resp = await client.post(`/campaigns/${campaignId}/adgroups/find`, selector);
            }
            else {
                resp = await client.get(`/campaigns/${campaignId}/adgroups`, { limit: String(limit ?? 50), offset: String(offset ?? 0) });
            }
...
Defaults: status=ENABLED, pricingModel=CPC, startTime=today. The defaultBidAmount is used for keywords that don't have an individual bid. Set cpaGoal to enable Apple's cost-per-acquisition optimization.`,
        ...
        try {
            ...
            if (cpaGoal)
                body.cpaGoal = { amount: cpaGoal, currency: cur };
            if (endTime)
                body.endTime = toISO8601(endTime);
            const resp = await client.post(`/campaigns/${campaignId}/adgroups`, body);
            return {
                content: [
    ...
    server.registerTool("update_ad_group", {
        ...
        try {
            ...
            if (status)
                body.status = status;
            if (defaultBidAmount)
                body.defaultBidAmount = { amount: defaultBidAmount, currency: cur };
            if (cpaGoal)
                body.cpaGoal = { amount: cpaGoal, currency: cur };
            if (endTime)
            ...
            const resp = await client.put(`/campaigns/${campaignId}/adgroups/${adGroupId}`, body);
            return {
                content: [
    ...
    server.registerTool("delete_ad_group", {
        ...
        try {
            await client.delete(`/campaigns/${campaignId}/adgroups/${adGroupId}`);
            return {
    ...
    server.registerTool("get_ad_group", {
        ...
        try {
            const resp = await client.get(`/campaigns/${campaignId}/adgroups/${adGroupId}`);
            const ag = resp.data;
            const text = [
                `Ad Group: ${ag.name} (ID: ${ag.id})`,
                `Status: ${ag.displayStatus}`,
    ...
    server.registerTool("get_adgroup_report", {
