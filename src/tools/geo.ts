...
export function registerGeoTools(server: McpServer, client: AppleAdsClient) {
  server.registerTool("get_geo_performance", {
    ...
    try {
      ...
      const resp = await client.post<ReportResponse<{ countryOrRegion: string; campaignId: number }>>(
        "/reports/campaigns",
        reportReq
      );
